...
for i, aig in enumerate(input_rasters):
    # Skip raster if it's in skip_rasters list
    if aig in SKIP_RASTERS:
        logger.warning('[{0}/{1}] Skipping an empty'.format(i+1, n_files) +
    ...
    if len(sci_name) == 0:
        logger.warning('[{0}/{1}] No entry found'.format(i+1, n_files) +
        ...
        logger.info('[{0}/{1}] Translating {2} to {3}...'.format(i+1,
                                                                 n_files,
                                                                 aig,
                                                                 output_file))
        subprocess.run(cmd)
...
outfile_path = snakemake.output['data_manifest']
with open(outfile_path, 'w') as outfile:
    outfile.write(yaml.dump(data_manifest, default_flow_style=False))
logger.info('Created data manifest file {}'.format(outfile_path))
